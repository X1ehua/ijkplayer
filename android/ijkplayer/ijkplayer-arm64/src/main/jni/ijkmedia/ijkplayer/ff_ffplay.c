...
typedef long long int ll_int_t;
...
static void decoder_init(Decoder *d, AVCodecContext *codec_ctx, PacketQueue *queue, SDL_cond *empty_queue_cond) {
...
static int convert_image(FFPlayer *ffp, AVFrame *src_frame, int64_t src_frame_pts, int width, int height) {
    ...
    if (is->viddec.codec_ctx) {
        av_reduce(&display_aspect_ratio.num, &display_aspect_ratio.den,
            is->viddec.codec_ctx->width * (int64_t)is->viddec.codec_ctx->sample_aspect_ratio.num,
            is->viddec.codec_ctx->height * (int64_t)is->viddec.codec_ctx->sample_aspect_ratio.den,
            1024 * 1024);
    ...
    if (ret >= 0 && got_packet > 0) {
        ...
        strcat(file_path, "/");
        sprintf(file_name, "%lld", (ll_int_t)src_frame_pts);
...
static int decoder_decode_frame(FFPlayer *ffp, Decoder *d, AVFrame *frame, AVSubtitle *sub)
{
    ...
    for (;;) {
                    ...
                    case AVMEDIA_TYPE_AUDIO:
                        ...
                        if (ret >= 0) {
                            ...
                            if (frame->pts != AV_NOPTS_VALUE)
                                frame->pts = av_rescale_q(frame->pts, av_codec_get_pkt_timebase(d->codec_ctx), tb);
        ...
        if (pkt.data == flush_pkt.data) {
            ...
            if (d->codec_ctx->codec_type == AVMEDIA_TYPE_SUBTITLE) {
                int got_frame = 0;
                ret = avcodec_decode_subtitle2(d->codec_ctx, sub, &got_frame, &pkt);
                if (ret < 0) {
                    ret = AVERROR(EAGAIN);
                } else {
...
static void video_image_display2(FFPlayer *ffp)
{
    ...
    vp = frame_queue_peek_last(&is->pictq); // pictq: FrameQueue
...
static void stream_component_close(FFPlayer *ffp, int stream_index)
{
    ...
    fmt_ctx->streams[stream_index]->discard = AVDISCARD_ALL;
    switch (codecpar->codec_type) {
    case AVMEDIA_TYPE_AUDIO:
        is->audio_st = NULL;
        is->audio_stream = -1;
    ...
    if (ffp->show_status)
    {
        ...
        if (!last_time || (cur_time - last_time) >= 30000)
        {
            ...
            av_log(NULL, AV_LOG_INFO,
                   ...
                   vqsize / 1024,
                   sqsize,
                   is->video_st ? is->viddec.codec_ctx->pts_correction_num_faulty_dts : 0,
                   is->video_st ? is->viddec.codec_ctx->pts_correction_num_faulty_pts : 0);
            fflush(stdout);
            last_time = cur_time;
        }
    }
...
static int queue_picture(FFPlayer *ffp, const AVFrame *src_frame, double pts, double duration, int64_t pos, int serial)
{
            ...
            if ((pts * 1000 * 1000 < is->seek_pos) || deviation > MAX_DEVIATION) {
                ...
                if (is->drop_vframe_count == 0) {
                    ...
                    av_log(NULL, AV_LOG_INFO, "video accurate_seek start, is->seek_pos=%lld, pts=%lf, is->accurate_seek_time = %lld",
                           (ll_int_t)is->seek_pos, pts, (ll_int_t)is->accurate_seek_start_time);
                }
                ...
                if ((now - is->accurate_seek_start_time) <= ffp->accurate_seek_timeout) {
                    ...
                    av_log(NULL, AV_LOG_WARNING, "video accurate_seek is error, is->drop_vframe_count=%d, now = %lld, pts = %lf",
                           is->drop_vframe_count, (ll_int_t)now, pts);
                ...
                av_log(NULL, AV_LOG_INFO, "video accurate_seek is ok, is->drop_vframe_count =%d, is->seek_pos=%lld, pts=%lf",
                       is->drop_vframe_count, (ll_int_t)is->seek_pos, pts);
    ...
    if (!(vp = frame_queue_peek_writable(&is->pictq))) // vp = is->pictq->queue[ .windex ]
        return -1;
    ...
    if (!vp->bmp || !vp->allocated ||
        ...
        if (vp->width != src_frame->width || vp->height != src_frame->height) // vp.w vp.h: 0 0
        ...
        /* the allocation must be done in the main thread to avoid
           locking problems. */
        alloc_picture(ffp, src_frame->format); // format: 0 AV_PIX_FMT_YUV420P
    ...
    if (vp->bmp) {
        ...
        // reached: src_frame->format 10001[IJK_AV_PIX_FMT__ANDROID_MEDIACODEC]
        ...
        SDL_VoutUnlockYUVOverlay(vp->bmp); // SDL_UnlockMutex(vout_overlay->opaque->mutex);
                ...
                if (ffp->enable_accurate_seek && is->audio_accurate_seek_req && !is->seek_req) {
                        ...
                        if ((audio_clock * 1000 * 1000 < is->seek_pos ) || deviation > MAX_DEVIATION) {
                            if (is->drop_aframe_count == 0) {
                                ...
                                av_log(NULL, AV_LOG_INFO,
                                       ...
                                       (ll_int_t)is->seek_pos, audio_clock, (ll_int_t)is->accurate_seek_start_time);
                            }
                    ...
                    if (audio_accurate_seek_fail) {
                        av_log(NULL, AV_LOG_INFO, "audio accurate_seek is error, is->drop_aframe_count=%d, now = %lld, audio_clock = %lf",
                               is->drop_aframe_count, (ll_int_t)now, audio_clock);
                        is->drop_aframe_count       = 0;
                        SDL_LockMutex(is->accurate_seek_mutex);
                        is->audio_accurate_seek_req = 0;
                        SDL_CondSignal(is->video_accurate_seek_cond);
...
static int subtitle_thread(void *arg)
{
    ...
    for (;;) {
        ...
        if (got_subtitle) {
            ...
            sp->width = is->subdec.codec_ctx->width;
            sp->height = is->subdec.codec_ctx->height;
            sp->uploaded = 0;
...
static int stream_component_open(FFPlayer *ffp, int stream_index)
{
    ...
    if (ret < 0)
        goto fail;
    av_codec_set_pkt_timebase(codec_ctx, fmt_ctx->streams[stream_index]->time_base);

    codec = avcodec_find_decoder(codec_ctx->codec_id); /* avcodec_find_decoder() */
    ...
    if(stream_lowres > av_codec_get_max_lowres(codec)){
        av_log(codec_ctx, AV_LOG_WARNING, "The maximum value for lowres supported by the decoder is %d\n",
                av_codec_get_max_lowres(codec));
        stream_lowres = av_codec_get_max_lowres(codec);
    }
    av_codec_set_lowres(codec_ctx, stream_lowres);

#if FF_API_EMU_EDGE
    if(stream_lowres) codec_ctx->flags |= CODEC_FLAG_EMU_EDGE;
#endif
    if (ffp->fast)
    ...
    opts = filter_codec_opts(ffp->codec_opts, codec_ctx->codec_id, fmt_ctx, fmt_ctx->streams[stream_index], codec);
    ...
    case AVMEDIA_TYPE_AUDIO:
#if CONFIG_AVFILTER
        {
            ...
            is->audio_filter_src.freq           = codec_ctx->sample_rate;
            is->audio_filter_src.channels       = codec_ctx->channels;
            is->audio_filter_src.channel_layout = get_valid_channel_layout(codec_ctx->channel_layout, codec_ctx->channels);
            is->audio_filter_src.fmt            = codec_ctx->sample_fmt;
            SDL_LockMutex(ffp->af_mutex);
        ...
        if ((ret = audio_open(ffp, channel_layout, nb_channels, sample_rate, &is->audio_tgt)) < 0)
            goto fail;
        ffp_set_audio_codec_info(ffp, AVCODEC_MODULE_NAME, avcodec_get_name(codec_ctx->codec_id));
        ...
        is->audio_st = fmt_ctx->streams[stream_index];

        decoder_init(&is->auddec, codec_ctx, &is->audioq, is->continue_read_thread);
        if ((is->fmt_ctx->iformat->flags & (AVFMT_NOBINSEARCH | AVFMT_NOGENSEARCH | AVFMT_NO_BYTE_SEEK)) && !is->fmt_ctx->iformat->read_seek) {
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        is->video_st = fmt_ctx->streams[stream_index];
        ...
        if (ffp->async_init_decoder) {
            ...
            if (ffp->node_vdec) {
                is->viddec.codec_ctx = codec_ctx;
                ret = ffpipeline_config_video_decoder(ffp->pipeline, ffp);
            }
            if (ret || !ffp->node_vdec) {
                decoder_init(&is->viddec, codec_ctx, &is->videoq, is->continue_read_thread);
            ...
            decoder_init(&is->viddec, codec_ctx, &is->videoq, is->continue_read_thread);
            ffp->node_vdec = ffpipeline_open_video_decoder(ffp->pipeline, ffp); // NOTE>> import
            if (!ffp->node_vdec)
                goto fail;
        }
        ...
        if (is->is_video_high_fps) {
            codec_ctx->skip_frame       = FFMAX(codec_ctx->skip_frame, AVDISCARD_NONREF);
            codec_ctx->skip_loop_filter = FFMAX(codec_ctx->skip_loop_filter, AVDISCARD_NONREF);
            codec_ctx->skip_idct        = FFMAX(codec_ctx->skip_loop_filter, AVDISCARD_NONREF);
        }
    ...
    case AVMEDIA_TYPE_SUBTITLE:
        ...
        is->subtitle_st = fmt_ctx->streams[stream_index];

        ffp_set_subtitle_codec_info(ffp, AVCODEC_MODULE_NAME, avcodec_get_name(codec_ctx->codec_id));

        decoder_init(&is->subdec, codec_ctx, &is->subtitleq, is->continue_read_thread);
...
static int media_read_thread(void *arg)
{
    ...
    if (ffp->skip_calc_frame_rate) {
        av_dict_set_int(&fmt_ctx->metadata, "skip-calc-frame-rate", ffp->skip_calc_frame_rate, 0);
        av_dict_set_int(&ffp->format_opts, "skip-calc-frame-rate", ffp->skip_calc_frame_rate, 0);
    }

    if (ffp->iformat_name)
        is->iformat = av_find_input_format(ffp->iformat_name);
    err = avformat_open_input(&fmt_ctx, is->filename, is->iformat, &ffp->format_opts);
    if (err < 0) {
        ...
        do {
            if (av_stristart(is->filename, "data:", NULL) && orig_nb_streams > 0) {
                for (i = 0; i < orig_nb_streams; i++) {
                    if (!fmt_ctx->streams[i] || !fmt_ctx->streams[i]->codecpar || fmt_ctx->streams[i]->codecpar->profile == FF_PROFILE_UNKNOWN) {
                        break;
                    }
                }
    ...
    if (ffp->seek_by_bytes < 0)
        ffp->seek_by_bytes = !!(fmt_ctx->iformat->flags & AVFMT_TS_DISCONT) && strcmp("ogg", fmt_ctx->iformat->name);

    is->max_frame_duration = (fmt_ctx->iformat->flags & AVFMT_TS_DISCONT) ? 10.0 : 3600.0;
    is->max_frame_duration = 10.0;
    av_log(ffp, AV_LOG_INFO, "max_frame_duration: %.3f\n", is->max_frame_duration);

#ifdef FFP_MERGE
    if (!window_title && (t = av_dict_get(fmt_ctx->metadata, "title", NULL, 0)))
    ...
    for (i = 0; i < fmt_ctx->nb_streams; i++) {
        ...
        if (type >= 0 && ffp->wanted_stream_spec[type] && st_index[type] == -1)
            if (avformat_match_stream_specifier(fmt_ctx, st, ffp->wanted_stream_spec[type]) > 0)
                st_index[type] = i;
    ...
    for (;;) {
        ...
        if (is->paused != is->last_paused) {
            ...
            if (is->paused)
                is->read_pause_return = av_read_pause(fmt_ctx);
            else
        ...
        if (is->paused &&
                (!strcmp(fmt_ctx->iformat->name, "rtsp") ||
                 (fmt_ctx->pb && !strncmp(ffp->input_filename, "mmsh:", 5)))) {
...
//      of the seek_pos/seek_rel variables
            ...
            ret = avformat_seek_file(is->fmt_ctx, -1, seek_min, seek_target, seek_max, is->seek_flags);
            if (ret < 0) {
                av_log(NULL, AV_LOG_ERROR,
...
#ifdef FFP_MERGE
        #define max_size MAX_QUEUE_SIZE
#else
        #define max_size ffp->dcc.max_buffer_size /* dcc: FFDemuxCacheControl */
#endif

        /* if the queue are full, no need to read more */
        if (ffp->infinite_buffer<1 && !is->seek_req
            && (is->audioq.size + is->videoq.size + is->subtitleq.size > max_size || (
        ...
        if (ret < 0) {
            int pb_eof = 0;
            int pb_error = 0;
            if ((ret == AVERROR_EOF || avio_feof(fmt_ctx->pb)) && !is->eof) {
        ...
        if (pkt->flags & AV_PKT_FLAG_DISCONTINUITY) { // discontinuity 不连续 中断 间断 停顿
            if (is->audio_stream >= 0) {
                packet_queue_put(&is->audioq, &flush_pkt);
            }
            if (is->subtitle_stream >= 0) {
        ...
        stream_start_time = fmt_ctx->streams[pkt->stream_index]->start_time;
        pkt_ts = pkt->pts == AV_NOPTS_VALUE ? pkt->dts : pkt->pts;
        pkt_in_play_range = ffp->duration == AV_NOPTS_VALUE ||
                (pkt_ts - (stream_start_time != AV_NOPTS_VALUE ? stream_start_time : 0)) *
                av_q2d(fmt_ctx->streams[pkt->stream_index]->time_base) -
...
int ffp_seek_to_l(FFPlayer *ffp, long msec)
{
    ...
    start_time = is->fmt_ctx->start_time;
...
long ffp_get_current_position_l(FFPlayer *ffp)
{
    ...
    int64_t start_time = is->fmt_ctx->start_time;
...
long ffp_get_duration_l(FFPlayer *ffp)
{
    ...
    int64_t duration = fftime_to_milliseconds(is->fmt_ctx->duration);
    if (duration < 0)
        return 0;
...
{
    ALOGE(">> ffp_video_thread"); // not reached
...
int ffp_set_stream_selected(FFPlayer *ffp, int stream, int selected)
{
    ...
    if (stream < 0 || stream >= fmt_ctx->nb_streams) {
        av_log(ffp, AV_LOG_ERROR, "invalid stream index %d >= stream number (%d)\n", stream, fmt_ctx->nb_streams);
        return -1;
    }

    codecpar = fmt_ctx->streams[stream]->codecpar;
...
int ffp_start_record(FFPlayer *ffp, const char *file_name)
{
    ...
    if (!is || !is->fmt_ctx|| is->paused || is->abort_request) {
        av_log(ffp, AV_LOG_ERROR, "is,is->fmt_ctx,is->paused is invalid");
        goto end;
    }

    if (ffp->is_recording) {
        av_log(ffp, AV_LOG_ERROR, "already recording");
        goto end;
    }

    // 初始化用于输出的 AVFormatContext
    ...
    for (int i = 0; i < is->fmt_ctx->nb_streams; i++) {
        // 对照输入流创建输出流通道
        AVStream *in_stream = is->fmt_ctx->streams[i];
        ...
        // av_log(ffp, AV_LOG_DEBUG, "in_stream->codec 0x%x", (int)(in_stream->codec));
        if (avcodec_copy_context(out_stream->codec, in_stream->codec) < 0) {
            av_log(ffp, AV_LOG_ERROR, "Failed to copy context from input to output stream codec");
            goto end;
        }
...
{
    // 分两种情况：realtime-recording or cached-recording
    // 后者，需要重置

    assert(ffp);
    if (!ffp->is_recording) {
        av_log(ffp, AV_LOG_ERROR, "stopRecord() error: ffp->is_recording == 0");
        return -1;
    }

    if (ffp->cached_pkts.pkts) {
        for (int i=0; i<ffp->cached_pkts.arr_size; i++) {
            if (ffp->cached_pkts.pkts[i].size > 0) {
                av_packet_unref(& ffp->cached_pkts.pkts[i]);
            }
        }
        av_free(ffp->cached_pkts.pkts);
        ffp->cached_pkts.pkts = NULL;
    }
